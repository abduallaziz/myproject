...
if($heroF->getHeroStatus()!=100){
	...
	if($heroF->getHeroStatus() == 101){
		$deadTitle = "<span class='itemNotMoveable'>لا يمكنك إستعمال هذه الأداة وبطلك ميت.</span><br>";
	}else{
		$deadTitle = "<span class='itemNotMoveable'>بطلك خارج القرية.</span><br>";
	}
...
<div id="bodyOptions">
	<div id="hero_body_container">
		<div id="hero_body">
         ...
         <img class="heroBodyImage heroBodyImage-<?php echo DIRECTION; ?>"  src="<?=$database->heroBody($session->uid)?>" >
			<div class="clear"></div>
		</div>
...
if($gi['rightHand']!=0){
	...
	$item .= '<div id="item_'.$gi['rightHand'].'" title="'.$Travian->getItemData($btype, $type)['name'].'||'.$deadTitle.$Travian->getItemData($btype, $type)['title'].'" class="item item_'.$data['type'].' onHero'.$dis.'" style="position: relative; left: 0px; top: 0px; "><div class="amount">'.$data['num'].'</div></div>';
	if(!$dis){$item .= '</a>';}
...
include_once "application/views/Auction/alt.php";
	...
	//if($type != 0){
		$outputList .= "<div id=\"inventory_".$inv."\" class=\"inventory draggable\">";
		$outputList .= "<div id=\"item_".$id."\" title=\"".$amount."".$Travian->getItemData($btype, $type)['name']."||".$deadTitle."".$Travian->getItemData($btype, $type)['title']."\" class=\"item ".$gender."_item_".($Travian->getItemData($btype, $type)['item'])."".$dis."\" ' style=\"position:relative;left:0;top:0;\">";
...
<script type="text/javascript">
	Travian.Game.Hero.Inventory = new (new Class(
	{
		...
		alreadyOpen: false,
        textSingle: "<?=heroh1?>" ,
		textMulti: 'عدد القطع: &lt;input class=\"text\" id=\"amount\" type=\"text\" value=\"\" /&gt;'.unescapeHtml(),
		initialize: function() {
			var $this = this;

...
//print_r($queue);die;
    foreach($queue as $block) {
        ...
        if($block!='border'){
            imagecopyresized($img, imagecreatefromgif('img/m/'. $block.'.gif'), $x, $y, 0, 0, $bw, $bh, 60, 60); //обводка
        }else{
            imagecopyresized($img, imagecreatefromgif('img/m/borderneutr.gif'), $x, $y, 0, 0, $bw, $bh, 60, 60); //обводка
        }
        $x += $bw;
        $n++;
